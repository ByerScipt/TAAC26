...
class ModelInput(NamedTuple):
    ...
    # 当前样本日历特征，0 为 padding。
    hour: torch.Tensor           # [B], 1-24 (0=padding)
    day_of_week: torch.Tensor    # [B], 1-7 (0=padding, 1=Mon)
    day_of_month: torch.Tensor   # [B], 1-31 (0=padding)
...
class MultiSeqQueryGenerator(nn.Module):
    ...
    def __init__(
        ...
        self.d_model = d_model
        self.multi_scale_queries = multi_scale_queries
        if self.multi_scale_queries and self.num_queries != 3:
            raise ValueError("multi_scale_queries requires num_queries=3")
    ...
    @staticmethod
    def _prefix_horizon_masks(valid_mask: torch.Tensor) -> List[torch.Tensor]:
        """按有效长度切出前 10% / 前 70% / 全量三档视野。"""
        valid_len = valid_mask.sum(dim=1).float()
        cutoffs = [
            torch.ceil(valid_len * 0.10).long().clamp(min=1),
            torch.ceil(valid_len * 0.70).long().clamp(min=1),
            valid_len.long().clamp(min=1),
        ]
        positions = torch.arange(valid_mask.shape[1], device=valid_mask.device).unsqueeze(0)
        return [valid_mask & (positions < cutoff.unsqueeze(1)) for cutoff in cutoffs]

    @staticmethod
    def _pool_valid_tokens(tokens: torch.Tensor, valid_mask: torch.Tensor) -> torch.Tensor:
    ...
    def forward(
        ...
        for i in range(self.num_sequences):
            ...
            if not self.multi_scale_queries:
                ...
                # 三个 query 不再共用同一个全局池化结果，而是各自读取不同前缀范围。
                pooled_list = [
                    self._pool_valid_tokens(seq_tokens_list[i], view_mask)
                    for view_mask in self._prefix_horizon_masks(valid_mask)
                ]
                queries = []
                for q_idx, seq_pooled in enumerate(pooled_list):
                    # GlobalInfo_i = Concat(NS_flat, seq_pooled_i)
                    global_info = torch.cat([ns_flat, seq_pooled], dim=-1)  # (B, (M+1)*D)
                    global_info = self.global_info_norm(global_info)
...
class MultiSeqHyFormerBlock(nn.Module):
    ...
    def forward(
        ...
        seq_tokens_list: list,
        seq_padding_masks: list,
        time_tokens_list: Optional[List[torch.Tensor]] = None,
        ...
        Args:
            ...
            time_tokens_list: 可选的当前样本时间 token，每个历史域一枚。
                它只追加到 cross-attention 的 KV 侧，不进入序列 self-attention
                和 RankMixer。
        ...
        decoded_qs = []
        for i in range(S):
            # 当前样本时间只追加到 KV 侧，用来调节 query 的检索视野。
            kv = next_seqs[i]
            kv_mask = next_masks[i]
            if time_tokens_list is not None:
                kv = torch.cat([kv, time_tokens_list[i]], dim=1)  # (B, L+1, D)
                kv_mask = torch.cat([
                    kv_mask,
                    torch.zeros(kv_mask.shape[0], 1, dtype=torch.bool, device=kv_mask.device),
                ], dim=1)  # (B, L+1)，当前时间 token 始终可见

            rc = rope_cos_list[i] if rope_cos_list is not None else None
            rs = rope_sin_list[i] if rope_sin_list is not None else None
            if self.multi_scale_queries:
                base_valid = ~next_masks[i]
                query_outputs = []
                for q_idx, view_valid in enumerate(self._prefix_horizon_masks(base_valid)):
                    view_mask = ~view_valid
                    if time_tokens_list is not None:
                        # 当前样本时间不属于历史序列窗口，三路 query 都保留可见。
                        view_mask = torch.cat([
                            view_mask,
                            torch.zeros(view_mask.shape[0], 1, dtype=torch.bool, device=view_mask.device),
                        ], dim=1)
                    query_outputs.append(self.cross_attns[i](
                ...
                decoded_q_i = torch.cat(query_outputs, dim=1)
            else:
                decoded_q_i = self.cross_attns[i](
                    q_tokens_list[i], kv, kv_mask,
                    rope_cos=rc, rope_sin=rs,
                )
...
class PCVRHyFormer(nn.Module):
    ...
    def __init__(
        ...
        if rank_mixer_mode == 'full':
            if multi_scale_queries:
                logging.info(
                    f"multi_scale_queries enabled: forcing rank_mixer_mode "
                    f"from 'full' to 'ffn_only' "
                    f"(d_model={d_model}, T={num_queries}*{self.num_sequences}"
                    f"+{self.num_ns}={T}, d_model%%T={d_model % T})")
        ...
        # 日历特征词表和 dataset.py 保持同一份定义。
        from dataset import NUM_HOUR_BUCKETS, NUM_DOW_BUCKETS, NUM_DOM_BUCKETS
        # 不同历史域对同一个日历时间的含义不同，因此每个域单独学习当前时间查表。
        num_domains = self.num_sequences
        self.hour_embeddings = nn.ModuleList([
            nn.Embedding(NUM_HOUR_BUCKETS, d_model, padding_idx=0)
            for _ in range(num_domains)
        ])
        self.dow_embeddings = nn.ModuleList([
            nn.Embedding(NUM_DOW_BUCKETS, d_model, padding_idx=0)
            for _ in range(num_domains)
        ])
        self.dom_embeddings = nn.ModuleList([
            nn.Embedding(NUM_DOM_BUCKETS, d_model, padding_idx=0)
            for _ in range(num_domains)
        ])
        ...
        # Q token 汇总了历史兴趣，单独的 dropout 系数用于抑制时间漂移下的过拟合。
        self.q_dropout = nn.Dropout(dropout_rate * q_dropout_mult)
    ...
    def _init_params(self) -> None:
        ...
        # 日历 embedding 不参与高基数重置，初始化时只清 padding 行。
        for emb in self.hour_embeddings:
            nn.init.xavier_normal_(emb.weight.data)
            emb.weight.data[0, :] = 0
        for emb in self.dow_embeddings:
            nn.init.xavier_normal_(emb.weight.data)
            emb.weight.data[0, :] = 0
    ...
    def _build_domain_time_kv(
        self,
        hour: torch.Tensor,
        day_of_week: torch.Tensor,
        day_of_month: torch.Tensor,
    ) -> List[torch.Tensor]:
        """把当前样本时间压成每个历史域各自的一枚 KV 条件 token。"""
        tokens = []
        for i in range(self.num_sequences):
            token = (
                self.hour_embeddings[i](hour)
                + self.dow_embeddings[i](day_of_week)
                + self.dom_embeddings[i](day_of_month)
            )
    ...
    def _embed_seq_domain(
        ...
        注意：当前样本的 hour/day_of_week/day_of_month 不在这里加到事件 token 上。
        它们会经由 ``_build_domain_time_kv`` 进入 cross-attention 的 KV 侧，
        只影响 query 检索历史的方式，不进入 self-attention 和 RankMixer。
        """
        B, S, L = seq.shape
        emb_list = []
        for i in range(S):
        ...
        # Add time bucket embedding (per-position, not global)
        if self.num_time_buckets > 0:
    ...
    def _run_multi_seq_blocks(
        ...
        seq_tokens_list: list,
        seq_masks_list: list,
        time_tokens_list: Optional[list] = None,
        apply_dropout: bool = True
    ) -> torch.Tensor:
        """Runs the multi-sequence block stack with dropout and output projection."""
        if apply_dropout:
            q_tokens_list = [self.q_dropout(q) for q in q_tokens_list]
        ...
        for block in self.blocks:
            ...
            if self.rotary_emb is not None:
                ...
                # 追加当前时间 token 时，RoPE 缓存长度需要覆盖最后那个 KV 位置。
                extra = 1 if time_tokens_list is not None else 0
                for seq_i in curr_seqs:
                    seq_len = seq_i.shape[1]
                    cos, sin = self.rotary_emb(seq_len + extra, device)
            ...
            curr_qs, curr_ns, curr_seqs, curr_masks = block(
                ...
                time_tokens_list=time_tokens_list,
    ...
    def forward(self, inputs: ModelInput) -> torch.Tensor:
        ...
        # 当前时间只作为检索条件，不混进序列演化和 NS token 混合。
        time_tokens_list = self._build_domain_time_kv(
            inputs.hour, inputs.day_of_week, inputs.day_of_month)

        # 历史事件仍只承载自身 side-info 与时间差 bucket，避免被当前时间直接污染。
        seq_tokens_list = []
        seq_masks_list = []
        for domain in self.seq_domains:
    ...
    def predict(self, inputs: ModelInput) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # 推理阶段沿用训练时的时间条件路径，保持时间 token 只服务于 cross-attention。
        time_tokens_list = self._build_domain_time_kv(
            inputs.hour, inputs.day_of_week, inputs.day_of_month)
